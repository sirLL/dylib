...
//                circleBounds.bottom + (rimWidth / 2.0f) + (contourSize / 2.0f));
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressView_pwBarWidth, barWidth);
        rimWidth = (int) a.getDimension(R.styleable.ProgressView_pwRimWidth, rimWidth);
        spinSpeed = (int) a.getDimension(R.styleable.ProgressView_pwSpinSpeed, spinSpeed);
        barLength = (int) a.getDimension(R.styleable.ProgressView_pwBarLength, barLength);

        delayMillis = a.getInteger(R.styleable.ProgressView_pwDelayMillis, delayMillis);
        if (delayMillis < 0) {
            delayMillis = 10;
        }

        // Only set the text if it is explicitly defined
        if (a.hasValue(R.styleable.ProgressView_pwText)) {
            setText(a.getString(R.styleable.ProgressView_pwText));
        }

        barColor = a.getColor(R.styleable.ProgressView_pwBarColor, barColor);
        textColor = a.getColor(R.styleable.ProgressView_pwTextColor, textColor);
        rimColor = a.getColor(R.styleable.ProgressView_pwRimColor, rimColor);
        circleColor = a.getColor(R.styleable.ProgressView_pwCircleColor, circleColor);
        contourColor = a.getColor(R.styleable.ProgressView_pwContourColor, contourColor);

        textSize = (int) a.getDimension(R.styleable.ProgressView_pwTextSize, textSize);
        contourSize = a.getDimension(R.styleable.ProgressView_pwContourSize, contourSize);
