...
class RankingActivationAdapter : BaseAdapter<RankItemBean, RecyclerView.ViewHolder>() {
...
//            }
    ...
    private fun bindHeaderData(holder: HeaderViewHolder) {
        ...
        holder.tvAmountNo1.text = if (no1?.total.isNullOrBlank()) {
            "--"
        } else {
            no1?.total
        }
        ...
        holder.tvAmountNo2.text = if (no2?.total.isNullOrBlank()) {
            "--"
        } else {
            no2?.total
        }
        ...
        holder.tvAmountNo3.text = if (no3?.total.isNullOrBlank()) {
            "--"
        } else {
            no3?.total
        }
